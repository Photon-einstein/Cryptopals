...
std::string Server::aesCtrEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
    bool *b) {
      ...
      for (j = 0; j <  nBlocks; ++j, Server::updateIVCtrMode()) {
        ...
        for (i = 0; i < _blockSize && k < plainTextBytesAsciiFullText.size(); ++i, ++k) {
          plaintextPointer[i] = plainTextBytesAsciiFullText[i+j*_blockSize];
        }
        /* Encrypt the plaintext */
        encryptedTextLen = Server::aesCtrEncryptWorker(_iv, _blockSize,
        _key, _iv, cyphertextPointer);
        if (debugFlagExtreme == true) {
          std::cout<<"Full Decrypted text size = "<<encryptedTextLen<<" bytes."<<std::endl;
...
no errors or to false otherwise */
std::string Server::aesCtrDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
      bool *b) {
    ...
    for (j = 0; j <  nBlocks; ++j, Server::updateIVCtrMode()) {
      ...
      for (i = 0; i < _blockSize && k < encryptedBytesAsciiFullText.size(); ++i, ++k) {
        cypherTextPointer[i] = encryptedBytesAsciiFullText[i+j*_blockSize];
      }
      /* Decrypt the ciphertext */
      decryptedTextLen = Server::aesCtrDecryptWorker(_iv, _blockSize,
...
/******************************************************************************/
int Server::aesCtrEncryptWorker(unsigned char *plaintext, int plaintextLen, unsigned char *key,
...
/******************************************************************************/
int Server::aesCtrDecryptWorker(unsigned char *cyphertext, int cypherTextLen, unsigned char *key,
...
void Server::loadInputStrings(const std::string inputFilePath) {
  ...
  while(inputFile.good() == true) {
    ...
    if (lineReadBase64.size() > 0) {
      ...
      if (debugFlag == true) {
        std::cout<<"Line read (ascii): '"<<lineReadAscii<<"' | size = "<<lineReadAscii.size()<<".\n"<<std::endl;
      }
...
false otherwise */
std::string Server::decryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
    bool *b) {
  std::string decryptedText;
  if (encryptedBytesAsciiFullText.size() == 0) {
    *b = false;
  ...
  bool flag;
  if (debugFlagExtreme == true) {
    std::cout<<"Size ciphertext to decrypt: "<<encryptedBytesAsciiFullText.size()<<"'."<<std::endl;
  }
...
false otherwise */
std::string Server::encryption(const std::vector<unsigned char> &plaintextBytesAsciiFullText,
    bool *b) {
  std::string encryptedText;
  if (plaintextBytesAsciiFullText.size() == 0) {
    *b = false;
  ...
  bool flag;
  if (debugFlagExtreme == true) {
    std::cout<<"Size plaintext to encrypt: "<<plaintextBytesAsciiFullText.size()<<"'."<<std::endl;
  }
...
bool Server::testDecryptedVectorString(std::vector<std::string> decryptedTextV,
  ...
  for (i = 0; i < size; ++i) {
    ...
    if (aux != decryptedTextV[i]) {
      std::cout<<"\nMismatch between: [S]'"<<aux<<"' and [A]'"<<decryptedTextV[i]<<"'."<<std::endl;
      return false;
    }
  }
  return true;
