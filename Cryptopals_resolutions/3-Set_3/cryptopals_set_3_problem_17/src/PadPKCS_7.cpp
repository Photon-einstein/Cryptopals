...
bool PadPKCS_7::testPadding(std::vector<unsigned char> &v) {
  if (v.size() % _blockSize != 0) {
    printf("Bad Padding | Padded vector size must be a multiple of the block size.");
    return false;
  }
  int size = v.size(), i;
  ...
  if (lastC > _blockSize) {
    if (debugFlagExtreme) {
      printf("Bad Padding | Padded size cannot be greater than the block size.");
    }
    return false;
  }
  for (i = 0; i < lastC; ++i) {
    if (v[size-1-i] != lastC) {
      if (debugFlagExtreme == true) {
        printf("Bad Padding | The padding char should not change during the padding length.");
      }
      return false;
    }
  }
