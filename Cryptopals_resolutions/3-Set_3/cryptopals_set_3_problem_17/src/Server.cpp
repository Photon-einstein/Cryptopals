...
std::string Server::aesCbcEncryption(
  ...
  encryptedTextLen = Server::aesCbcEncryptWorker(plainTextPointer, size2, _key,
                                                 _iv, encryptedTextPointer);
  if (debugFlag == true) {
    std::cout << "Full Encrypted ECB text size = " << encryptedTextLen
              << " bytes." << std::endl;
...
std::string Server::aesCbcDecryption(
  ...
  decryptedTextLen = Server::aesCbcDecryptWorker(
      cypherTextPointer, size, key, iv, cypherTextDecryptedPointer);
  if (debugFlagExtreme == true) {
    std::cout << "Full Decrypted text size = " << decryptedTextLen << " bytes."
              << std::endl;
...
/******************************************************************************/
int Server::aesCbcEncryptWorker(unsigned char *plaintext, int plaintextLen,
...
/******************************************************************************/
int Server::aesCbcDecryptWorker(unsigned char *cyphertext, int cypherTextLen,
                                unsigned char *key, unsigned char *iv,
...
bool Server::encryptionSessionTokenAesCbcMode(
    std::vector<unsigned char> &ciphertextV, std::vector<unsigned char> &iv) {
  std::random_device rd;  // non-deterministic generator
  std::mt19937 gen(rd()); // to seed mersenne twister.
  std::uniform_int_distribution<> dist1(
      0, _stringsAscii.size() -
             1); // distribute results between 0 and 255 inclusive
  std::vector<unsigned char> plainTextBytesAsciiFullText;
  std::string ciphertext;
  int i = dist1(gen), j;
  bool b;
  if (debugFlagExtreme == true) {
    std::cout << "\nRandom string selected was: " << i + 1 << " | '"
              << _stringsAscii[i] << "' | size " << _stringsAscii[i].size()
              << " bytes.\n"
              << std::endl;
  }
  Function::convertStringToVectorBytes(_stringsAscii[i],
                                       plainTextBytesAsciiFullText);
  ciphertext = Server::aesCbcEncryption(plainTextBytesAsciiFullText, _blockSize,
                                        _key, _iv, &b);
  if (b == false) {
...
bool Server::decryptAndCheckPaddingInSessionTokenAesCbcMode(
    const std::vector<unsigned char> ciphertextV, bool *returnValue) {
  ...
  plaintext =
      Server::aesCbcDecryption(ciphertextV, _blockSize, _key, _iv, &flag);
  if (flag == false) {
...
bool Server::checkPresenceOfValidSessionToken(
    const std::string &possibleSessionToken) {
