...
void Server::setSeed() {
  ...
  std::uniform_int_distribution<> dist(
      0, INT_MAX); // distribute results between 0 and INT_MAX inclusive
  _currentSeed =
      dist(gen) & 0x0000ffff; // downsize the seed to 16 bits in this problem
...
void Server::setRandomPrefixSize() {
  ...
  std::mt19937 gen(rd()); // to seed mersenne twister.
  std::uniform_int_distribution<> dist(
      0, _maxRandomNumberLetters); // distribute results between 0 and
                                   // maxRandonNumberLetters inclusive
...
std::vector<unsigned char> Server::encryptWithStreamCypherBasedOnMt19937() {
  ...
  if (_numberLettersEncryptedWithSameSeed + size >
      _maxCharactersMt19937WithSameSeed) {
    Server::setSeed();
    _numberLettersEncryptedWithSameSeed = 0;
  }
...
Server::encryptWithStreamCypherBasedOnMt19937(std::string plaintext) {
  if (debugFlag == true) {
    std::cout << "Encrypt" << std::endl;
  }
  unsigned char c;
...
std::string Server::decryptWithStreamCypherBasedOnMt19937(
...
std::string Server::getKnownPlaintext() {
  ...
  std::mt19937 gen(rd()); // to seed mersenne twister.
  std::uniform_int_distribution<> dist(
      0, UCHAR_MAX); // distribute results between 0 and UCHAR_MAX inclusive
  int i;
...
std::string Server::generatePossiblePasswordToken() {
  ...
  std::uniform_int_distribution<> dist1(
      0, 1); // distribute results between 0 and 1 inclusive
  ...
  std::uniform_int_distribution<> dist2(
      0, _maxRandomNumberLetters *
             5); // distribute results between 0 and 1 inclusive
  ...
  if (dist1(gen1) == 0) {
     ...
     */
    Server::setSeed();
    if (_numberLettersEncryptedWithSameSeed + sizeString >
        _maxCharactersMt19937WithSameSeed) {
      Server::setSeed();
      _numberLettersEncryptedWithSameSeed = 0;
    }
...
given by the attacker were correct or not, if it were correct then it will
...
bool Server::checkAttackerAnswer(const std::string &passwordToken,
  ...
  if (_mPasswordToken.count(passwordToken) == 0) {
    std::cout << "Server log | password reset token "
              << " is not present in the database." << std::endl;
    return false;
  }
  // case 2: the veredict of the attacker does not match the server's database
  if (_mPasswordToken[passwordToken].useMt19937Flag !=
      idAnswer.useMt19937Flag) {
    std::cout << "Server log | password reset token " << " false veredict."
              << std::endl;
    return false;
  }
  ...
  if (_mPasswordToken[passwordToken].useMt19937Flag == false) {
    std::cout << "Server log | password reset token "
              << " true veredict | no PRNG MT19937 was used." << std::endl;
    return true;
  }
  ...
  if (_mPasswordToken[passwordToken].seedMt19937 != idAnswer.seedMt19937) {
    std::cout << "Server log | password reset token "
              << " false veredict | PRNG MT19937's seed incorrectly guessed."
              << std::endl;
    return true;
  } else {
    /* case 5: bool veredict and seed were correct, as both matches database
      values */
    std::cout
        << "Server log | password reset token "
        << " true veredict | PRNG MT19937's seed correctly guessed | seed = "
