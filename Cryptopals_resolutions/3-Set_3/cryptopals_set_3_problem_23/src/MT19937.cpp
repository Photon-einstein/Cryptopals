...
/******************************************************************************/
/* initialize the generator from the state vector */
void MT19937::seedMt(const std::vector<unsigned int> &stateVector) {
  if (stateVector.size() != _n) {
    throw std::invalid_argument("State vector with wrong size passed to 'MT19937::seedMt()' function.");
  }
  std::size_t i;
  for (i = 0; i < _n; ++i) {
      _mt[i] = stateVector[i];
...
bool MT19937::checkEqualVectorState(const std::vector<std::uint32_t>
    &_mt19937StateVector) {
  ...
  for (i = 0; i < _n; ++i) {
    if (_mt[i] != _mt19937StateVector[i]) {
      std::cout<<"(_mt["<<i<<"] = "<<_mt[i]<<") != (_mt19937StateVector["<<i<<"] = "<<_mt19937StateVector[i]<<")"<<std::endl;
      return false;
    }
  }
  return true;
