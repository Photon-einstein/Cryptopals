...
void Attacker::extractNumbersFromMt19937HomeMadeBeforeAttack() {
  ...
  for (i = 0; i < _internalStateValueSize; ++i) {
    ...
    if (debugFlag == true && i < maxSizeDebug) {
      std::cout << "(sample) Attacker log read from server | _mt19937Values["
                << i << "] = " << _mt19937Values[i] << std::endl;
    }
  }
  std::cout << "\nAttacker log | " << _internalStateValueSize
            << " values read.\n"
            << std::endl;
  return;
}
...
std::uint32_t Attacker::reverseShiftRightXor(std::uint32_t value,
                                             std::uint8_t shift) {
...
mask */
std::uint32_t Attacker::reverseShiftLeftXor(std::uint32_t value,
                                            std::uint8_t shift,
                                            std::uint32_t mask) {
  // iterate until we've done the full 32 bits
  for (size_t i = 0; i * shift < 32; ++i) {
...
bool Attacker::recoverStateMt19937() {
  ...
  for (i = 0; i < _internalStateValueSize; ++i) {
    ...
    if (debugFlag == true && i < maxSizeDebug) {
      std::cout << "(sample) attacker log mt19937 state vector | _mt_attacker["
                << i << "] = " << value << std::endl;
    }
...
bool Attacker::testRecoverStateMt19937() {
  ...
  for (i = 0; i < _internalStateValueSize; ++i) {
    ...
    if (nTest != nServer) {
      std::cout << "Attacker log test | mt19937_atck[" << i << "] = " << nTest
                << " != mt19937_srv[" << i << "] = " << nServer << " | failed."
                << std::endl;
      return false;
    } else if (debugFlag == true && i < maxSizeDebug) {
      std::cout << "(sample new values) Attacker log test | mt19937_attacker["
                << i << "] = " << nTest
                << " | equals new server value | passed." << std::endl;
    }
  }
  if (debugFlag == true) {
    std::cout << std::endl;
...
bool Attacker::cloneMt19937() {
  ...
  if (b == false) {
    std::cout << "There was an error in the function "
                 "Attacker::testRecoverStateMt19937.";
    return false;
  }
  return true;
}
