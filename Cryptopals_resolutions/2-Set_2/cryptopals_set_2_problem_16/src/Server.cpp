...
std::string Server::aesCbcEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
  ...
  flag = Function::fillVectorFromPointerArray(previousCypherTextVector, iv, blockSize);
  if (flag == false) {
  ...
  for (i = 0; i < nCycles; ++i) {
    ...
    for (j = 0; j < (int)blockSize; ++j) {
      plainTextVector.push_back(plainTextBytesAsciiFullTextCopy[blockSize*i+j]);
    }
    /* previous cyphertext XOR plainText */
    flag = Function::xorFunction(previousCypherTextVector, plainTextVector, xorRes);
    if (flag == false) {
    ...
    if (debugFlagExtreme == true) {
      std::cout<<"Round encrypted text size = "<<encryptedTextLen<<std::endl;
    }
    ...
    flag = Function::fillVectorFromPointerArray(previousCypherTextVector, encryptedTextPointer, blockSize);
    if (flag == false) {
      perror("\nThere was an error in the function 'fillVectorFromPointerArray'.");
      *b = false;
      return encryptedText;
...
std::string Server::aesCbcDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
  ...
  flag = Function::fillVectorFromPointerArray(previousCypherText, iv, blockSize);
  if (flag == false) {
    perror("\nThere was an error in the function 'fillVectorFromPointerArray'.");
    *b = false;
    return decryptedText;
  ...
  for (i = 0; i < nCycles; ++i) {
    ...
    /* Decrypt the ciphertext */
    decryptedTextLen = Server::aesEcbDecryptWorker(cypherTextPointer, blockSize,
    key, iv, cypherTextDecryptedPointer);
    if (debugFlagExtreme == true) {
      std::cout<<"Round decrypted text size = "<<decryptedTextLen<<std::endl;
    }
    /* Add a NULL terminator. We are expecting printable text */
    cypherTextDecryptedPointer[decryptedTextLen] = '\0';
    /* fill cyphertext vector */
    flag = Function::fillVectorFromPointerArray(cypherText, cypherTextDecryptedPointer, decryptedTextLen);
    if (flag == false) {
      perror("\nThere was an error in the function 'fillVectorFromPointerArray'.");
      *b = false;
      return decryptedText;
    ...
    /* previousCypherText = cypherText */
    flag = Function::fillVectorFromPointerArray(previousCypherText, cypherTextPointer, blockSize);
    if (flag == false) {
      perror("\nThere was an error in the function 'fillVectorFromPointerArray'.");
      *b = false;
      return decryptedText;
...
/******************************************************************************/
int Server::aesEcbEncryptWorker(unsigned char *plaintext, int plaintextLen, unsigned char *key,
...
/******************************************************************************/
int Server::aesEcbDecryptWorker(unsigned char *cyphertext, int cypherTextLen, unsigned char *key,
...
bool Server::processInput(std::string data, std::string &inputProcessed) {
  ...
  if (debugFlag == true) {
    std::cout<<"\nData: '"<<data<<"' processed becomes: '"<<dataCleaned<<"'."<<std::endl;
    std::cout<<"\nProcessed input: '"<<processedData<<"', size = "<<processedData.size()<<"."<<std::endl;
  }
  Function::convertStringToVectorBytes(processedData, plainTextBytesAsciiFullText);
  encryptedString = aesCbcEncryption(plainTextBytesAsciiFullText, _blockSize, _key, _iv, &flag);
  if (flag == false) {
...
bool Server::testEncryption(const std::string &encryption, bool *res) {
  ...
  decryptedText = Server::aesCbcDecryption(encryptedBytesAsciiFullText, _blockSize, _key, _iv, &flag);
  if (flag == false) {
