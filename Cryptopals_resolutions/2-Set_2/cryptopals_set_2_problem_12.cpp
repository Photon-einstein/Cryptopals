...
// To compile: $ g++ -Wall -std=c++17 cryptopals_set_2_problem_12.cpp -o cryptopals_set_2_problem_12 -lcrypto
...
int aesEcbEncryptWorker(unsigned char *plaintext, int plaintextLen, const unsigned char *key,
            const unsigned char *iv, unsigned char *cyphertext);
...
int main (void) {
  ...
  /* work to verify */
  bool flag;
  std::string encryptionMode, inputFileName="cryptopals_set_2_problem_12_dataset.txt";
  std::string encryptedText, decryptedTextString;
  ...
  if (flag == false) {
    ...
    printf("The function 'getBlockCypherSize' could not find a valid block size up to %d.", maxBlockSize);
    exit(1);
  } else {
    std::cout<<"Block size calculated: "<<blockSizeCalculated<<"."<<std::endl;
  }
...
bool getDecodeDataFromFile(const std::string inputFileName,
  ...
  while(inputFile.good() == true) {
    ...
    b = decodeBase64ToByte(lineReadBase64Vector, base64IndexMap, inputBytesAscii);
    if (b == false) {
...
bool populateDictionary(std::map<std::string, unsigned char> &dictionary,
    const std::vector<unsigned char> &knownStringV, const int blockSize,
    unsigned char *key, unsigned char *iv) {
  if ((int)knownStringV.size() != blockSize-1 || blockSize < 1 || key == nullptr ||
    iv == nullptr) {
      return false;
  }
  int i, j;
...
bool decryptText(std::vector<unsigned char> &unknownStringV,
  ...
  for (i = 0; i < nRounds; ++i) {
    ...
    encryptedText = aesEcbEncryption(plaintextFullVector, blockSize, keyV, iv, &flag);
    if (flag == false) {
      perror("There was a problem in the function 'aesEcbEncryption'.");
      exit(1);
    } else {
    ...
    flag = decryptTextRound(knownStringV, encryptedTextV, blockSize, keyV, iv, decryptedText);
    if (flag == false) {
      perror("There was a problem in the function 'decryptText'.");
      exit(1);
    }
...
bool decryptTextRound(const std::vector<unsigned char> &knownStringV,
  ...
  flag = populateDictionary(dictionary, previousVectorKnow, blockSize, keyV, iv);
  if (flag == false) {
