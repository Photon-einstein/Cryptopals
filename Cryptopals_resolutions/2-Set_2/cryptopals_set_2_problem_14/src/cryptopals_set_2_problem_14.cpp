...
int main (void) {
  ...
  flag = Function::guessSizeRandomPrefix(randomPrefixWork, sizeRandomPrefixGuess, blockSize);
  if (flag == false) {
    perror("There was a problem in the function 'guessSizeRandomPrefix'.");
    exit(1);
  } else if (randomPrefixWork->testRandomPrefixSize(sizeRandomPrefixGuess) == true) {
    printf("\nThe function 'guessSizeRandomPrefix' correctly estimated the size of the random prefix used in AES ecb mode as %d bytes.", sizeRandomPrefixGuess);
  } else {
    printf("\nThe function 'guessSizeRandomPrefix' failed estimated the size of the random prefix used in AES ecb mode, stating %d bytes.", sizeRandomPrefixGuess);
  }
  ...
  flag = Function::decryptText(unknownStringV, blockSize, keyV, iv, decryptedTextString, randomPrefixWork, sizeRandomPrefixGuess);
  if (flag == false) {
