...
int main (void) {
  ...
  if (flag == false) {
    ...
    printf("\nThe function 'guessSizeRandomPrefix' correctly estimated the size of the random prefix used in AES ecb mode as %d bytes.\n", sizeRandomPrefixGuess);
  } else {
    printf("\nThe function 'guessSizeRandomPrefix' failed estimated the size of the random prefix used in AES ecb mode, stating %d bytes.\n", sizeRandomPrefixGuess);
  }
  ...
  flag = Function::decryptText(unknownStringV, blockSize, decryptedTextString, randomPrefixWork, sizeRandomPrefixGuess);
  if (flag == false) {
