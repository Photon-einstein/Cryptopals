...
bool Function::keyFilling(const int blockSize, std::string &keyMain) {
  if (blockSize < 1) {
    return false;
  }
  key.clear();
...
bool Function::plaintextFilling(std::vector<unsigned char> &v, const int size) {
  if (blockSize < 1) {
    return false;
  }
  int i;
...
bool Function::populateDictionary(std::map<std::string, unsigned char> &dictionary,
  ...
  for (i = 0; i < 255; ++i) {
    ...
    b = Function::padPKCS_7(plaintextVPadded, blockSize);
    if (b == false) {
      perror("There was a problem in the function 'padPKCS_7'");
      return false;
    }
    encryptedText = Function::aesEcbEncryption(plaintextVPadded, blockSize, key, iv, &b);
    if (b == false) {
...
bool Function::decryptText(std::vector<unsigned char> &unknownStringV,
  ...
  if (blockSize < 1 || randomPrefixWork.get() == nullptr || iv == nullptr || keyV == nullptr || sizeRandomPrefixGuess < 0 || sizeRandomPrefixGuess > blockSize) {
    return false;
  }
  ...
  for (i = 0; i < nRounds; ++i) {
    ...
    flag = Function::decryptTextRound(knownStringV, encryptedTextV, blockSize, keyV, iv, decryptedText);
    if (flag == false) {
...
bool Function::decryptTextRound(const std::vector<unsigned char> &knownStringV,
  ...
  flag = Function::populateDictionary(dictionary, previousVectorKnow, blockSize, keyV, iv);
  if (flag == false) {
...
oracleID Function::encryptionOracle(std::string plaintext, const int blockSize, bool *b) {
  ...
  Function::convertStringToVectorBytes(completePlainText, completePlainTextV);
  /* pad message */
  b1 = Function::padPKCS_7(completePlainTextV, blockSize);
  if (b1 == false) {
...
bool Function::guessSizeRandomPrefix(const std::shared_ptr<RandomPrefixWorker>&randomPrefixWork,
  ...
  bool flag;
  int i, notEqualFlag;
  /* insertion of the first byte */
  p1.push_back(0);
  p2.push_back(1);
  while (true) {
    /* clean ret1 and ret2 to the next cycle */
    ret1.clear();
    ret2.clear();
