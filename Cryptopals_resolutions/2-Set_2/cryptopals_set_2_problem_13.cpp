...
int main (void) {
  ...
  std::string key, testEncryptionString, encodedUserProfile;
  int i;
  ...
  * 'aesEcbDecryption' */
  ...
  b = ecbAttack(encodedUserProfile);
  if (b == false) {
    perror("There was an error in the function 'ecbAttack'.");
    exit(1);
  }
  std::cout<<"\nThe encoded user profile retrieved from the function 'ecbAttack' was: '"
...
bool profileForOracleEncrypt(const std::string &email, std::string
  ...
  convertStringToVectorBytes(email, plainTextBytesAsciiFullText);
  /* pad message */
  b = padPKCS_7(plainTextBytesAsciiFullText, blockSize);
  if (b == false) {
    perror("There was an error in the function 'padPKCS_7'.");
