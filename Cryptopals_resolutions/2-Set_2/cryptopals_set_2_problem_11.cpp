...
// To compile: $ g++ -Wall -std=c++11 cryptopals_set_2_problem_11.cpp -o cryptopals_set_2_problem_11 -lcrypto
...
int main (void) {
  ...
  for(i = 0; i < nTests; ++i) {
    ...
    std::cout<<"Test nº"<<i+1<<"\tOracle mode: "<<id.encryptionMode<<"\tDetector mode: "
      <<detectorVeredict<<"\t"<<veredict<<std::endl;
  }
  /* end of the work */
  end = clock();
...
std::string aesEcbEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
  ...
  if (debugFlag == true) {
    std::cout<<"Full Eecrypted text size = "<<encryptedText.size()<<std::endl;
  }
  return encryptedText;
}
/******************************************************************************/
...
std::string aesCbcEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
  ...
  for (i = 0; i < nCycles; ++i) {
    ...
    key, iv, encryptedTextPointer);
    if (debugFlag == true) {
      std::cout<<"Full Decrypted CBC text size = "<<encryptedTextLen<<std::endl;
      BIO_dump_fp (stdout, (const char *)encryptedTextPointer, encryptedTextLen);
    }
    /* Add a NULL terminator. We are expecting printable text */
    encryptedTextPointer[encryptedTextLen] = '\0';
...
oracleID encryptionOracle(std::string plaintext, const int blockSize, bool *b) {
  ...
  std::uniform_int_distribution<> dist1(0,255), dist2(0,1); // distribute results between 0 and 255 inclusive.
  int i;
  bool b1;
