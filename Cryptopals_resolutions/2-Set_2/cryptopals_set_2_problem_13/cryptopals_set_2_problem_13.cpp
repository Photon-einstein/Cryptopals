...
// To compile: $ g++ -Wall -std=c++17 cryptopals_set_2_problem_13.cpp -o
// cryptopals_set_2_problem_13 -lcrypto
...
bool printJsonFormat(const std::string &structuredCookie,
...
/* this function does the decryption of aes-ecb mode using the iv and key
values, in the end it returns the decrypted text and sets flag b by reference to
true if no errors or to false otherwise */
std::string
aesEcbDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
                 unsigned int blockSize, unsigned char *key, unsigned char *iv,
                 bool *b);
...
true if all ok or false otherwise */
bool profileForOracleEncrypt(const std::string &email,
                             std::string &encodedStringOutputEncrypted);

/* this function makes an attack into aes ecb mode, trying to forge an admin
user role to the email of the attacker, in the end it will return the
encodedUserProfile by reference and true if all went ok, false if there was an
error */
...
int main(void) {
  ...
  std::cout << "\nEncoded email='" << nastyEmail << "' with role user as: '"
            << encodedStringOutput << "'." << std::endl;
  ...
  /* step 5.1: Encrypt the encoded user profile under the key ->
   * 'aesEcbEncryption' */
  ...
  testEncryptionString =
      aesEcbEncryption(plainTextBytesAsciiFullText, blockSize, keyV, iv, &b);
  if (b == false) {
    perror("There was an error in the function 'aesEcbEncryption'.");
    exit(1);
  }
  /* step 5.2: Decrypt the encoded user profile and parse it ->
   * 'aesEcbDecryption' */
  convertStringToVectorBytes(testEncryptionString, encryptedBytesAsciiFullText);
  testEncryptionString =
      aesEcbDecryption(encryptedBytesAsciiFullText, blockSize, keyV, iv, &b);
  if (b == false) {
  ...
  if (correctEmail == testEncryptionString) {
    std::cout << "\nThe functions 'aesEcbEncryption' and 'aesEcbDecryption' "
                 "are working well, test passed, string tested: '"
              << testEncryptionString
              << "', size = " << testEncryptionString.size() << "."
              << std::endl;
  } else {
    std::cout << "\nThe functions 'aesEcbEncryption' and 'aesEcbDecryption' "
                 "aren't working well, test failed."
              << std::endl;
  }
  /* step 6: test the proper function of 'aesEcbEncryption' and
   * 'aesEcbDecryption' */
  ...
  /* step 7: decrypt the encodedUserProfile retrieved from the ecbAttack
   * function */
  convertStringToVectorBytes(encodedUserProfileEncrypted,
                             encryptedBytesAsciiFullText);
  encodedUserProfile =
      aesEcbDecryption(encryptedBytesAsciiFullText, blockSize, keyV, iv, &b);
  if (b == false) {
  ...
  std::cout << "\nThe encoded user profile retrieved from the function "
               "'ecbAttack' was, cookie style: '"
...
bool printJsonFormat(const std::string &structuredCookie,
  ...
  int i, size = v.size();
  std::cout << "Structured cookie '" << structuredCookie
            << "' converted to json format as:" << std::endl;
  for (i = 0; i < size; ++i) {
...
bool profileFor(const std::string &email, std::string &encodedStringOutput) {
  ...
  std::uniform_int_distribution<> dist(
      10, 99); // distribute results between 0 and 255 inclusive
  ...
  encodedStringOutput = "email=" + emailAux +
                        "&uid=" + std::to_string(dist(gen)) + "&role=" + "user";
  return true;
}
/******************************************************************************/
/* this function makes the random filling of a key of size = blockSize, in the
...
aesEcbDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
  ...
  for (i = 0; i < nCycles; ++i) {
    ...
    decryptedTextLen = aesEcbDecryptWorker(cypherTextPointer, blockSize, key,
                                           iv, cypherTextDecryptedPointer);
    if (debugFlag == true) {
      std::cout << "Iterative decrypted text size = " << decryptedTextLen
                << ", block = " << i + 1 << "." << std::endl;
    }
    /* Add a NULL terminator. We are expecting printable text */
    cypherTextDecryptedPointer[decryptedTextLen] = '\0';
    /* fill cyphertext vector */
    flag = fillVectorFromPointerArray(cypherText, cypherTextDecryptedPointer,
                                      decryptedTextLen);
    if (flag == false) {
      perror(
          "\nThere was an error in the function 'fillVectorFromPointerArray'.");
  ...
  if (debugFlag == true) {
    std::cout << "Full Decrypted text size after unpadding = "
              << decryptedText.size() << std::endl;
  }
  return decryptedText;
}
/******************************************************************************/
...
bool profileForOracleEncrypt(const std::string &email,
  ...
  if (debugFlag == true) {
    ...
    for (i = 0; i < (int)encodedStringOutput.size(); ++i) {
      if (isalpha(encodedStringOutput[i]) || encodedStringOutput[i] == '@' ||
          encodedStringOutput[i] == '.' || encodedStringOutput[i] == '=' ||
          encodedStringOutput[i] == '&') {
...
bool ecbAttack(std::string &encodedUserProfileEncrypted) {
  ...
  bool b;
  if (debugFlag == true) {
    std::cout << "Number of 'o' in the email will be equal to " << nOs
              << std::endl;
  }
  for (i = 0; i < nOs; ++i) {
    attackMail += "o";
  ...
  if (debugFlag == true) {
    ...
    for (i = 0; i < (int)attackMail.size(); ++i) {
      if (isalpha(attackMail[i]) || attackMail[i] == '@' ||
          attackMail[i] == '.') {
  ...
  for (i = 0; i < nBlocks; ++i) {
    for (j = 0; j < (int)blockSize; ++j) {
      if (i == 0) {
        ...
        attackMailEncrypted[(i + 1) * blockSize + j] =
            encodedStringOutputEncrypted[i * blockSize + j];
      } else {
        attackMailEncrypted[(i - 1) * blockSize + j] =
            encodedStringOutputEncrypted[i * blockSize + j];
      }
