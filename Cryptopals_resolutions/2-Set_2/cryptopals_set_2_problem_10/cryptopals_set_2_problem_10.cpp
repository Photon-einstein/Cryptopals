...
// To compile: $ g++ -Wall -std=c++11 cryptopals_set_2_problem_10.cpp -o
// cryptopals_set_2_problem_10 -lcrypto
...
true if no errors or to false otherwise */
std::string
aesCbcDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
                 unsigned int blockSize, unsigned char *key, unsigned char *iv,
                 bool *b);
...
int main(void) {
  ...
  memcpy(key, "YELLOW SUBMARINE", strlen("YELLOW SUBMARINE") + 1);
  unsigned char *iv =
      (unsigned char *)calloc(2 * blockSize + 1, sizeof(unsigned char));
  ...
  if (!inputFile) {
    ...
    std::cout << "The file 'cryptopals_set_2_problem_10_dataset.txt' was "
                 "sucessfully opened."
              << std::endl;
  }
  /* base64IndexMap */
  ...
  decryptedText =
      aesCbcDecryption(encryptedBytesAsciiFullText, blockSize, key, iv, &b);
  if (b == false) {
...
aesCbcEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
  ...
  for (i = 0; i < nCycles; ++i) {
    ...
    encryptedTextLen =
        encrypt(plainTextPointer, blockSize, key, iv, encryptedTextPointer);
    if (debugFlag == true) {
...
aesCbcDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
  ...
  for (i = 0; i < nCycles; ++i) {
    ...
    /* Decrypt the ciphertext */
    decryptedTextLen = decrypt(cypherTextPointer, blockSize, key, iv,
                               cypherTextDecryptedPointer);
    if (debugFlag == true) {
    ...
    /* previousCypherText = cypherText */
    flag = fillVectorFromPointerArray(previousCypherText, cypherTextPointer,
                                      blockSize);
    if (flag == false) {
      perror(
          "\nThere was an error in the function 'fillVectorFromPointerArray'.");
...
bool testEncryptDecrypt() {
  unsigned char *key =
      (unsigned char *)calloc(2 * blockSize + 1, sizeof(unsigned char));
  memcpy(key, "YELLOW SUBMARINE",
         strlen("YELLOW SUBMARINE") + 1); //(unsigned char *)"YELLOW SUBMARINE";
  unsigned char *iv =
      (unsigned char *)calloc(2 * blockSize + 1, sizeof(unsigned char));
  bool b;
  ...
  std::string stringTest =
      "Ehrsam, Meyer, Smith and Tuchman invented the cipher\n";
  stringTest += "block chaining (CBC) mode of operation in 1976.[23] In CBC "
                "mode, each block\n";
  stringTest += "of plaintext is XORed with the previous ciphertext block "
                "before being encrypted.\n";
  stringTest += "This way, each ciphertext block depends on all plaintext "
                "blocks processed up to\n";
  stringTest += "that point. To make each message unique, an initialization "
                "vector must be used\n";
