 ...
 */
const std::string EncryptionUtility::generateCryptographicNonce(
 ...
 */
std::vector<unsigned char> EncryptionUtility::generateRandomIV(std::size_t ivLength) {
    std::vector<unsigned char> iv(ivLength);
    if (RAND_bytes(iv.data(), ivLength) != 1) {
        throw std::runtime_error(
          "EncryptionUtility log | generateRandomIV(): "
