...
false otherwise */
std::string AesCtrMachine::encryption(const std::vector<unsigned char> &plaintextBytesAsciiFullText,
    bool *b) {
  ...
  if (debugFlagExtreme == true) {
    std::cout<<"AesCtrMachine log | size plaintext to encrypt: "<<plaintextBytesAsciiFullText.size()<<"'."<<std::endl;
  }
  encryptedText = AesCtrMachine::aesCtrEncryption(plaintextBytesAsciiFullText, &flag);
  *b = flag;
  return encryptedText;
}
/******************************************************************************/
...
false otherwise */
std::string AesCtrMachine::decryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
    bool *b) {
  ...
  bool flag;
  if (debugFlagExtreme == true) {
    std::cout<<"AesCtrMachine log | size ciphertext to decrypt: "<<encryptedBytesAsciiFullText.size()<<"'."<<std::endl;
  }
  //AesCtrMachine::resetIVCtrMode();
  decryptedText = AesCtrMachine::aesCtrDecryption(encryptedBytesAsciiFullText, &flag);
  *b = flag;
  return decryptedText;
}
/******************************************************************************/
...
std::string AesCtrMachine::aesCtrEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
    bool *b) {
  std::string encryptedText;
  if (plainTextBytesAsciiFullText.size() == 0) {
    *b = false;
  ...
  plaintextPointer = (unsigned char*) calloc(_blockSize+1, sizeof (unsigned char));
  if (plaintextPointer == nullptr) {
    perror("There was a problem in the memory allocation of the 'cyperTextPointer'.");
    *b = false;
    return encryptedText;
  }
  cyphertextPointer = (unsigned char*) calloc(_blockSize+1, sizeof (unsigned char));
  ...
  for (j = 0; j <  nBlocks; ++j, AesCtrMachine::updateIVCtrMode()) {
    ...
    encryptedTextLen = AesCtrMachine::aesCtrEncryptWorker(_iv, _blockSize,
    _key, _iv, cyphertextPointer);
    if (debugFlagExtreme == true) {
      std::cout<<"AesCtrMachine log | full decrypted text size = "<<encryptedTextLen<<" bytes."<<std::endl;
    ...
    flag = Function::appendToVectorFromPointerArray(decryptedTextVector, cyphertextPointer, encryptedTextLen);
    if (flag == false) {
...
no errors or to false otherwise */
std::string AesCtrMachine::aesCtrDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
    bool *b) {
  std::string decryptedText;
  if (encryptedBytesAsciiFullText.size() == 0) {
    *b = false;
  ...
  cypherTextDecryptedPointer = (unsigned char*) calloc(_blockSize+1, sizeof (unsigned char));
  ...
  for (j = 0; j <  nBlocks; ++j, AesCtrMachine::updateIVCtrMode()) {
    ...
    decryptedTextLen = AesCtrMachine::aesCtrDecryptWorker(_iv, _blockSize,
    _key, _iv, cypherTextDecryptedPointer);
    if (debugFlagExtreme == true) {
      std::cout<<"AesCtrMachine log | full decrypted text size = "<<decryptedTextLen<<" bytes."<<std::endl;
    ...
    /* fill cyphertext vector */
    flag = Function::appendToVectorFromPointerArray(decryptedTextVector, cypherTextDecryptedPointer, decryptedTextLen);
    if (flag == false) {
      perror("\nThere was an error in the function 'fillVectorFromPointerArray'.");
      *b = false;
      return decryptedText;
...
/******************************************************************************/
int AesCtrMachine::aesCtrEncryptWorker(unsigned char *plaintext, int plaintextLen, unsigned char *key,
...
/******************************************************************************/
int AesCtrMachine::aesCtrDecryptWorker(unsigned char *cyphertext, int cypherTextLen, unsigned char *key,
...
/******************************************************************************/
/* getters */
int AesCtrMachine::getBlockSize() {
  return _blockSize;
}
/******************************************************************************/
unsigned char* AesCtrMachine::getKey() {
  return _key;
}
/******************************************************************************/
unsigned char* AesCtrMachine::getIV() {
  return _iv;
}
/******************************************************************************/
