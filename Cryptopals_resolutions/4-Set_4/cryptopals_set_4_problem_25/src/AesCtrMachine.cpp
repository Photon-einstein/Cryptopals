...
std::string AesCtrMachine::aesCtrEncryption(const std::vector<unsigned char> &plainTextBytesAsciiFullText,
  ...
  for (j = 0; j <  nBlocks; ++j, AesCtrMachine::updateIVCtrMode()) {
    ...
    if (debugFlagExtreme == true) {
      std::cout<<"AesCtrMachine | full decrypted text size = "<<encryptedTextLen<<" bytes."<<std::endl;
...
std::string AesCtrMachine::aesCtrDecryption(const std::vector<unsigned char> &encryptedBytesAsciiFullText,
  ...
  for (j = 0; j <  nBlocks; ++j, AesCtrMachine::updateIVCtrMode()) {
    ...
    if (debugFlagExtreme == true) {
      std::cout<<"AesCtrMachine | full decrypted text size = "<<decryptedTextLen<<" bytes."<<std::endl;
