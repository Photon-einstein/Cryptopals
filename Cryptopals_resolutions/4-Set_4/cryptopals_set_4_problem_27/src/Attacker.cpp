...
bool Attacker::getKeyFromServer() {
  std::vector<unsigned char> plainTextBytesAsciiFullTextV, cipherTextV, p1, p3,
      keyV;
  ...
  if (b == false) {
    perror(
        "Attacker log | Error when calling the function 'Server::encryption'.");
    return false;
  }
...
void Attacker::setBlockSize(int blockSize) {
  if (blockSize < 1) {
    throw std::invalid_argument(
        "Attacker log | Bad blockSize: blockSize cannot be less than 1");
  }
  _blockSize = blockSize;
}
/******************************************************************************/
/* getters */
int Attacker::getBlockSize() { return _blockSize; }
/******************************************************************************/
