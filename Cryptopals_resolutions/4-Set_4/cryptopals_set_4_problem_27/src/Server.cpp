...
std::string Server::decryptionWithHighOrderCharTest(std::vector<unsigned char> &encryptedBytesAsciiFullText,
    bool *b) {
  ...
  *b = flag;
  if (*b == false) {
    perror("Server log | There was a problem in the function 'Server::decryption'.");
    return decryptedText;
  }
  ...
  if (flag == false) {
    ...
    std::cout<<"Server log | Erasing content of decrypted text as no high order chars were found, size decryptedText = "<<decryptedTextV.size()<<"."<<std::endl;
    decryptedText = "";
  }
  return decryptedText;
}
...
false otherwise */
std::string Server::decryption(std::vector<unsigned char> &encryptedBytesAsciiFullText,
    bool *b) {
  std::string decryptedText;
  std::vector<unsigned char> decryptedTextV;
  if (b == nullptr) {
    ...
    perror("Server log | Ciphertext should not be empty and should be a multiple of blockSize.");
    *b = false;
    return decryptedText;
  }
  bool flag;
  if (debugFlagExtreme == true) {
    std::cout<<"Server log | Size ciphertext to decrypt: "<<encryptedBytesAsciiFullText.size()<<"'."<<std::endl;
  }
  decryptedText = _aesCbcMachine->decryption(encryptedBytesAsciiFullText, &flag);
  /* return final values */
  *b = flag;
  return decryptedText;
}
...
void Server::setBlockSize(int blockSize) {
  if (blockSize < 1) {
    throw std::invalid_argument("Server log | Bad blockSize: blockSize cannot be less than 1");
  }
