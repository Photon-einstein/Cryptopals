...
bool Function::appendToVectorFromPointerArray(std::vector<unsigned char> &previousCypherText,
  ...
  if (previousCypherTextPointer == nullptr) {
    perror("Function log | Error when calling the function 'Function::appendToVectorFromPointerArray', null pointer exception.");
    return false;
  }
  int i;
...
bool Function::fillVectorFromPointerArray(std::vector<unsigned char> &previousCypherText,
  ...
  if (previousCypherTextPointer == nullptr) {
    perror("Function log | Error when calling the function 'Function::fillVectorFromPointerArray', null pointer exception.");
    return false;
  }
...
bool Function::xorFunction(const std::vector<unsigned char> &vS1, const std::vector<unsigned char> &vS2,
  ...
  if (vS1.size() != vS2.size()) {
    perror("Function log | Error when calling the function 'Function::xorFunction', different vector sizes.");
    return false;
  }
  int i, size = vS1.size();
  vRes.clear();
