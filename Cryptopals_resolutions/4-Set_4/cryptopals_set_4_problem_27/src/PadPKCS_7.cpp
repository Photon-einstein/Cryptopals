...
bool PadPKCS_7::testPadding(std::vector<unsigned char> &v) {
  if (v.size() % _blockSize != 0) {
    throw std::invalid_argument("PadPKCS_7 log | Bad Padding: Padded vector size must be a multiple of the block size.");
  }
  int size = v.size(), i;
  unsigned char lastC = v[size-1];
  if (lastC > _blockSize) {
    throw std::invalid_argument("PadPKCS_7 log | Bad Padding: Padded size cannot be greater than the block size.");
  }
  for (i = 0; i < lastC; ++i) {
    if (v[size-1-i] != lastC) {
      throw std::domain_error("PadPKCS_7 log | Bad Padding: The padding char should not change during the padding length.");
    }
  }
