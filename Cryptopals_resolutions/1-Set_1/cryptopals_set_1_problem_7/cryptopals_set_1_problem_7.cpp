...
// To compile: $ g++ -Wall -std=c++11 cryptopals_set_1_problem_7.cpp -o
// cryptopals_set_1_problem_7 -lcrypto
...
int main(void) {
  ...
  if (!inputFile) {
    ...
    std::cout << "The file 'cryptopals_set_1_problem_7_dataset.txt' was "
                 "sucessfully opened."
              << std::endl;
  }
  /* base64IndexMap */
  ...
  cypherText =
      (unsigned char *)calloc(cypherTextLen + 1, sizeof(unsigned char));
  if (cypherText == nullptr) {
    perror("There was a problem in the memory allocation of the 'cyperText' "
           "pointer.");
    exit(1);
  }
