...
*/
bool convertBytesToBase64(const std::vector<unsigned char> &outputBytesAscii,
                          std::vector<unsigned char> &decodeVectorBase64);
...
int main() {
  ...
  start = clock();
  /* work to verify */
  std::string s = "49276d206b696c6c696e6720796f757220627261696e206c696b65206120"
                  "706f69736f6e6f7573206d757368726f6f6d",
              stringDecodedBase64;
  const std::string base64Answer =
      "SSdtIGtpbGxpbmcgeW91ciBicmFpbiBsaWtlIGEgcG9pc29ub3VzIG11c2hyb29t";
  std::vector<unsigned char> outputBytesAscii = decodeHexToByte(s),
                             decodeVectorBase64;
  std::cout << "Ascii text converted: ";
  printVectorBytes(outputBytesAscii);
  convertBytesToBase64(outputBytesAscii, decodeVectorBase64);
  ...
  std::cout << "Answer string to compare: " << base64Answer
            << " with size = " << base64Answer.size() << std::endl;
  std::cout << "Result string to compare: " << stringDecodedBase64
            << " with size = " << stringDecodedBase64.size() << std::endl;
...
bool convertBytesToBase64(const std::vector<unsigned char> &outputBytesAscii,
                          std::vector<unsigned char> &decodeVectorBase64) {
  unsigned char base64char[4] = {0}, dummyChar = 0;
  int i, sizeOutputBytesAscii = outputBytesAscii.size(), j;
  for (i = 0; i < sizeOutputBytesAscii; i += 3) {
    ...
    if (i + 1 < sizeOutputBytesAscii) {
      base64char[1] = ((outputBytesAscii[i] & 0x03) << 4) |
                      (outputBytesAscii[i + 1] >> 4 & 0x0f);
    } else {
      base64char[1] =
          ((outputBytesAscii[i] & 0x03) << 4) | (dummyChar >> 4 & 0x0f);
    }
    /* third octet to base64 char conversion */
    if (i + 1 < sizeOutputBytesAscii && i + 2 < sizeOutputBytesAscii) {
      base64char[2] = ((outputBytesAscii[i + 1] & 0x0f) << 2) |
                      (outputBytesAscii[i + 2] >> 6 & 0x03);
    } else if (i + 1 < sizeOutputBytesAscii) {
      base64char[2] =
          ((outputBytesAscii[i + 1] & 0x0f) << 2) | (dummyChar >> 6 & 0x03);
    } else {
