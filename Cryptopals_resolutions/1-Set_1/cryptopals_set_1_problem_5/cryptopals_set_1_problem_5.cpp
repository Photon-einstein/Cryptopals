...
/* this function makes the xor of the plaintextVectorBytes with the
keyBytesVector and in the end it returns the vector with the cypherText and sets
true in the bool b if all ok, otherwise it sets to false */
std::vector<unsigned char>
xorWithCypher(const std::vector<unsigned char> &plaintextVectorBytes,
              const std::vector<unsigned char> &keyBytesVector, bool *b);
...
/* this function makes the conversion of the vector sAscii from Ascii format
into the hexadecimal format, in the vector vHex, in the end it just returns */
...
int main() {
  ...
  std::string cypherTextHexSolution =
      ...
      "282f";
  std::vector<unsigned char> plaintextVectorBytes, keyBytesVector,
      cypherTextVector;
...
xorWithCypher(const std::vector<unsigned char> &plaintextVectorBytes,
  ...
  int i, j, sizePlaintext = plaintextVectorBytes.size(),
            sizeKey = keyBytesVector.size();
